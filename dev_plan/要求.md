...
**安全要求**:
- **密码安全**: 用户密码在数据库中只存储hash值，登录时客户端发送计算好的hash到后端，后端使用hash进行校验和登录
- **API密钥加密**: API密钥使用用户登录密码进行加密存储，使用时先由客户端使用登录密码自动解密并发送回服务器再继续使用
...
前端：使用flutter，只需要构建web前端即可，不要构建其他平台。
...
通过以上配置，您可以在Flutter构建的Web页面中成功集成智谱GLM的搜索API功能，为用户提供强大的AI搜索体验。
